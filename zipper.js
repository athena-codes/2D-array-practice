// Write a function `zipper` that accepts two arrays as arguments. The function should return a 2D
// array containing pairs of elements at the same indices. You can assume that the arrays have the
// same length.

let zipper= function(array1, array2) {
   let pairs= []

   for (let i = 0; i < array1.length; i++) {    // one loop because pairs are not unique, but matching indicies
    let ele1 = array1[i]       // define first element index
    let ele2 = array2[i]       // define second element index
    let pair = [ ele1, ele2 ]  // new array for element 1 and 2 to go
       pairs.push(pair)   // push matching paired elements into empty array
   }
   return pairs    // return now filled array
 }

let array1 = ["a", "b", "c", "d"];
let array2 = [-1, -2, -3, -4];
console.log(zipper(array1, array2));
// [
//   ['a', -1],
//   ['b', -2],
//   ['c', -3],
//   ['d', -4],
// ]

let array3 = ["whisper", "talk", "shout"];
let array4 = ["quiet", "normal", "loud"];
console.log(zipper(array3, array4));
// [
//   ['whisper', 'quiet'],
//   ['talk', 'normal'],
//   ['shout', 'loud'],
// ]
